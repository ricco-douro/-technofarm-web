...
<?php
	...
	$read_data_associate_models = function(&$active_model, $all) use (&$read_data_associate_models, $dbo){
		foreach($all as $model_n => $model){
			if($model['related_to'] == $active_model->alias){
				...
				if(!empty($model['pkey'])){
					$relation_settings['pkey'] = $model['pkey'];
				}
				...
				if($relation == 'subqueryJoin'){
					...
					$active_model->$relation($new_model, $model['model_name'], $relation_conditions, $relation_settings);
				}
			}
		}
	};
	...
	if(!empty($function['conditions'])){
		...
		foreach($function['conditions'] as $condition){
			if($condition['_type'] == 'rule' AND !empty($condition['name'])){
				$filter_value = $this->Parser->parse($condition['value'], true);
	...
	if(!empty($function['orders'])){
		
		foreach($function['orders'] as $order){
			$Model->order([$order['name'] => !empty($order['action']) ? $order['action'] : 'asc']);
		}
