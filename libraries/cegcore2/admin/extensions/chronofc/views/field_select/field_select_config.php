...
<div class="ui segment tab views-tab active" data-tab="view-<?php echo $n; ?>">
	...
	<div class="ui bottom attached tab segment" data-tab="view-<?php echo $n; ?>-advanced">
		...
		<div class="two fields">
			<div class="field">
				...
				<small><?php el('The form event to be accessed for getting the auto complete results, the results must be a json encoded array.'); ?></small>
			</div>
			<div class="field">
				...
				<small><?php el('Minimum number of characters to be typed for the auto complete fetaure to start.'); ?></small>
			</div>
		</div>
		...
		<div class="equal width fields">
			<div class="field">
				<div class="ui checkbox toggle">
					<input type="hidden" name="Connection[views][<?php echo $n; ?>][searchable]" data-ghost="1" value="">
					<input type="checkbox" class="hidden" name="Connection[views][<?php echo $n; ?>][searchable]" value="1">
					<label><?php el('Searchable'); ?></label>
					<small><?php el('The dropdown values can be searched by typing in the field.'); ?></small>
				</div>
			</div>
			<div class="field">
				<div class="ui checkbox toggle">
					<input type="hidden" name="Connection[views][<?php echo $n; ?>][fulltextsearch]" data-ghost="1" value="">
					<input type="checkbox" class="hidden" name="Connection[views][<?php echo $n; ?>][fulltextsearch]" value="1">
					<label><?php el('Full text searchable'); ?></label>
					<small><?php el('Instead of matching the first character only, the whole option label text will be matched.'); ?></small>
				</div>
			</div>
			<div class="field">
				<div class="ui checkbox toggle">
					<input type="hidden" name="Connection[views][<?php echo $n; ?>][allowadditions]" data-ghost="1" value="">
					<input type="checkbox" class="hidden" name="Connection[views][<?php echo $n; ?>][allowadditions]" value="1">
					<label><?php el('Allow additions'); ?></label>
					<small><?php el('The user can add values to the dropdown values by typing in the field.'); ?></small>
				</div>
			</div>
		</div>
	...
	<div class="ui bottom attached tab segment small fields_events_list" data-tab="view-<?php echo $n; ?>-events">
		<?php $this->view(dirname(dirname(__FILE__)).DS.'field_events'.DS.'field_events_config.php', ['view' => $view, 'n' => $n, 'events_events' => ['=' => rl('Selecting'), '!=' => rl('unSelecting'), 'empty' => rl('Empty'), 'not_empty' => rl('Not empty'), 'regex' => rl('Matches RegEx'), 'change' => rl('Change')]]); ?>
	</div>
