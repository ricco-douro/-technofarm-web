...
class Field extends \G2\L\Helper{
	...
	function setup($type, $view, &$Parser, &$Html){
		...
		*/
		if(!empty($view['description']['text'])){
			$Html->desc($Parser->parse($view['description']['text'], true));
		}
		...
		*/
		if(!empty($view['tooltip']['text'])){
			$tooltip = '<i class="'.$view['tooltip']['class'].'" data-hint="'.htmlspecialchars($Parser->parse($view['tooltip']['text'], true)).'"></i>';
		...
		if(!empty($view['allowadditions'])){
			$Html->addClass('search selection');
			$Html->attr('data-allowadditions', 1);
		}
		
		if(!empty($view['fulltextsearch'])){
			$Html->attr('data-fulltextsearch', 1);
		}
	...
	function parseBasics(&$view, &$field_data, &$Parser){
		foreach(['label', 'name', 'id', 'placeholder', 'value', 'content', 'src', 'href'] as $attr){
			if(isset($field_data[$attr])){
				$field_data[$attr] = $Parser->parse($field_data[$attr], true);
			}
			if(isset($view[$attr])){
				$view[$attr] = $Parser->parse($view[$attr], true);
			}
		}
	}
	...
	function setDynamics($view, &$Parser, &$Html){
		$dynamics = !empty($view['dynamics']) ? $view['dynamics'] : [];
		if(!empty($view['validation'])){
			$dynamics['validation'] = ['enabled' => true];
		}
		...
		if(!empty($connection['alias'])){
			foreach($dynamics as $type => $dynamic){
				if(!empty($dynamic['enabled'])){
					$en_array = [];
					//$en_array['name'] = $field_data['name'];
					$en_array['name'] = str_replace('[]', '[-N-]', $field_data['name']);
					$en_array['type'] = $view['type'];
					$en_array['label'] = isset($view['label']) ? $view['label'] : $field_data['name'];
					if($view['type'] == 'field_file'){
						$en_array['extensions'] = $view['extensions'];
					}
					if(!empty($Html->options)){
						$en_array['options'] = $Html->options;
					}
					if($type == 'validation'){
						$en_array['validation'] = $view['validation'];
					}
					if($this->get('__multiplier_source')){
						//skip repeater source item
						$en_array['name'] = str_replace($this->get('__multiplier_source'), '-N-', $en_array['name']);
						$en_array['label'] = str_replace($this->get('__multiplier_source'), '-N-', $en_array['label']);
						$en_array['multiplier'] = true;
						//pr($en_array['name']);
						//continue;
					}
					if($this->get('__multiplier_clone')){
						//skip repeater clone items
						continue;
					}
					/*
					$stored = \GApp::session()->get($connection['alias'].'.'.$type.'.'.$view['name'], []);
					$names = \G2\L\Arr::getVal($stored, '[n].name', []);
					if(!in_array($en_array['name'], $names)){
						$stored[] = $en_array;
						\GApp::session()->set($connection['alias'].'.'.$type.'.'.$view['name'], $stored);
					}
					*/
					\GApp::session()->set($connection['alias'].'.'.$type.'.'.$view['name'], [$en_array]);
					/*
					...
					*/
					...
					if(isset($stored[$view['name']])){
						unset($stored[$view['name']]);
	...
	function setEvents($events, &$Parser, &$Html){
		...
		foreach($events as $field_event){
			if(!empty($field_event['identifier'])){
				...
				$field_event['identifier'] = [];
				foreach($ids as $id){
					$field_event['identifier'][] = $Parser->parse(trim($id), true);
				}
				//$field_event['identifier'] = $Parser->parse($field_event['identifier'], true);
				
				if(isset($field_event['value']) AND strlen(trim($field_event['value']))){
					$values = explode("\n", $field_event['value']);
					$field_event['value'] = [];
					foreach($values as $value){
						$field_event['value'][] = $Parser->parse(trim($value), true);
					}
				}
