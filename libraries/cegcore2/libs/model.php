...
class Model {
	...
	function __construct($settings = array()){
		if(!empty($this->apps) AND !in_array(\G2\Globals::get('app'), $this->apps)){
			$this->enabled = false;
			return $this;
		}
	...
	private function _relationsMany($rels, $data, $multi = true){
		...
		foreach($rels as $rel){
			...
			foreach($list as $k => $relation){
				...
				$pkey = !empty($settings['pkey']) ? $settings['pkey'] : $this->pkey;
				
				if($multi){
					$ids = Arr::getVal($data, array('[n]', $this->alias, $pkey), []);
				}else{
					$ids = Arr::getVal($data, array($this->alias, $pkey), []);
				}
