...
<div class="joms-stream__body no-head">
	<p>
		...
		if(!is_null($you))
		{
			...
			echo JText::sprintf('COM_COMMUNITY_STREAM_MY_FRIENDS', $other->getDisplayName(false, true), CUrlHelper::userLink($other->id));
		?>
		<?php
		} else {
			// @todo: use sprintf with language code
			echo JText::sprintf('COM_COMMUNITY_STREAM_OTHER_FRIENDS', $user1->getDisplayName(false, true),$user2->getDisplayName(false, true), CUrlHelper::userLink($user1->id), CUrlHelper::userLink($user2->id));
		}
		?>
	</p>
	<?php
