...
Class CGroupsAccess implements CAccessInterface
{
	...
	static public function groupsEdit($userId, $groupId, $group)
	{
		...
		if( $userId == 0 ) {
			...
			return false;
        // ACL check
		} else if( !$group->isAdmin($userId) && !CFactory::getUser()->authorise('community.groupedit', 'com_community')) {
	...
	{
		if (!CFactory::getUser()->authorise('community.groupdelete', 'com_community') && !($userId == $group->ownerid)) {
			return false;
		} else {
			return true;
		}
	...
	{  
		...
		if( $group->isAdmin( $userId , $groupId ) || CFactory::getUser()->authorise('community.postcommentedit', 'com_community') || $userId == $wall->post_by ) {
			return true;
		} else {
			return false;
		}
    ...
    static public function groupsPhotosCreate($userId, $groupId)
    {
        ...
        // FALSE globally disabled
        if(!$config->get('groupphotos') || !CFactory::getUser()->authorise('community.photocreate', 'com_community')) {
            echo "<!-- ".__FUNCTION__.__LINE__."-->";
            return false;
        }
