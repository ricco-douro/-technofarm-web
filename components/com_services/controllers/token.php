 ...
 * @since  1.6
 */
class ServicesControllerToken extends JControllerLegacy
{
	/**
	...
	public function edit()
	{
		...
		// Get the model.
		$model = $this->getModel('Token', 'ServicesModel');
	...
	public function publish()
	{
		...
		if ($user->authorise('core.edit', 'com_services') || $user->authorise('core.edit.state', 'com_services'))
		{
			...
			if (!$item)
			{
				...
				$this->setRedirect(JRoute::_('index.php?option=com_services&view=tokens', false));
			}
			else
			{
	...
	public function remove()
	{
		...
		if ($user->authorise('core.delete', 'com_services'))
		{
			...
			else
			{
				...
				$app->setUserState('com_services.edit.inventory.id', null);
				$app->setUserState('com_services.edit.inventory.data', null);

				$app->enqueueMessage(JText::_('COM_SERVICES_ITEM_DELETED_SUCCESSFULLY'), 'success');
				$app->redirect(JRoute::_('index.php?option=com_services&view=tokens', false));
			}
