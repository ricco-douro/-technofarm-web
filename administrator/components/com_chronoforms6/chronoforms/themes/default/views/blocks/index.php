...
<form action="<?php echo r2('index.php?ext=chronoforms&cont=blocks'); ?>" method="post" name="admin_form" id="admin_form" class="ui form">
	...
	<div class="ui">
		<div class="ui labeled icon dropdown button compact pointing blue">
			...
			<div class="menu">
				<a class="item" href="<?php echo r2('index.php?ext=chronoforms&cont=blocks&act=edit&Block[type]=views'); ?>">
					<?php el('Views block'); ?>
				</a>
				<a class="item" href="<?php echo r2('index.php?ext=chronoforms&cont=blocks&act=edit&Block[type]=functions'); ?>">
					<?php el('Functions block'); ?>
				</a>
			</div>
		</div>
	...
	<table class="ui selectable table attached">
		<thead>
			<tr>
				...
				<th class="ten wide"><?php echo $this->Sorter->link(rl('Title'), 'block_title'); ?></th>
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<?php echo $this->Html->attr('href', r2('index.php?ext=chronoforms&cont=blocks&act=edit'.rp('id', $Block['Block'])))->content($Block['Block']['title'])->tag('a'); ?>
					<br>
					<small><?php echo nl2br($Block['Block']['desc']); ?></small>
				</td>
				<!--<td class="collapsing"><?php echo $Block['Block']['block_id']; ?></td>-->
				<td class="collapsing">
					<?php if($Block['Block']['type'] == 'views'): ?>
						<div class="ui label blue">
					<?php elseif($Block['Block']['type'] == 'functions'): ?>
						<div class="ui label red">
					<?php endif; ?>
					<?php echo ucfirst($Block['Block']['type']); ?>
					</div>
				</td>
				<td class="collapsing">
					<?php if(!empty($Block['Block']['group'])): ?>
					<div class="ui label black"><?php echo $Block['Block']['group']; ?></div>
					<?php endif; ?>
				</td>
				<td>
					<?php
