...
class com_servicesInstallerScript
{
	...
	public function install($parent)
	{
		...
		if(JFolder::create(JPATH_ROOT . '/' . $newfolder)) {
			JFolder::create(JPATH_ROOT . '/' . $newfolder . '/v1');
			JFile::move(JPATH_ROOT . '/components/com_services/assets/v1/.htaccess',
				JPATH_ROOT . '/' . $newfolder . '/v1/.htaccess');
			JFile::move(JPATH_ROOT . '/components/com_services/assets/v1/index.php',
				JPATH_ROOT . '/' . $newfolder . '/v1/index.php');
		}
	}
	...
	public function update($parent)
	{
		...
		if(JFolder::exists(JPATH_ROOT . '/' . $newfolder . '/v1')){
			JFile::delete(JPATH_ROOT . '/' . $newfolder . '/v1/.htaccess');
			JFile::delete(JPATH_ROOT . '/' . $newfolder . '/v1/index.php');
			JFile::move(JPATH_ROOT . '/components/com_services/assets/v1/.htaccess',
				JPATH_ROOT . '/' . $newfolder . '/v1/.htaccess');
			JFile::move(JPATH_ROOT . '/components/com_services/assets/v1/index.php',
				JPATH_ROOT . '/' . $newfolder . '/v1/index.php');
		}elseif(JFolder::create(JPATH_ROOT . '/' . $newfolder)) {
			JFolder::create(JPATH_ROOT . '/' . $newfolder . '/v1');
			JFile::move(JPATH_ROOT . '/components/com_services/assets/v1/.htaccess',
				JPATH_ROOT . '/' . $newfolder . '/v1/.htaccess');
			JFile::move(JPATH_ROOT . '/components/com_services/assets/v1/index.php',
		...
		if(JFolder::exists(JPATH_ROOT . '/libraries/services/capi/dlid')){
			JFile::delete(JPATH_ROOT . '/libraries/services/capi/dlid/capidlid.php');
			JFolder::delete(JPATH_ROOT . '/libraries/services/capi/dlid');
		}
	}
	 ...
	 *
	 * Ensure that cAPI Core service plugins packaged with pkg_capi_core will be activated on Package Install.
	 *
	...
	public function postflight($type, $parent)
	{
		...
		if (strtolower($type) === 'install')
		{
			...
			foreach($servicesPlugins as $key => $value){

				while(JPluginHelper::getPlugin('services',$key) === null){
					sleep(3);
				}
				...
				$conditions = array(
					$db->quoteName('element') . ' = ' . $db->quote($key),
					$db->quoteName('folder') . ' = ' . $db->quote('services'),
