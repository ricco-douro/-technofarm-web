<?php
// namespace administrator\components\com_gdpr\libraries\framework\controller;
/**
 *
 ...
 * * @since 1.0
 */
class GdprController extends JControllerLegacy {
	/**
	...
	/**
	 * Method to check if you can save the configuration.
	 *
	 * @param array $data
	 *        	An array of input data.
	 * @param string $key
	 ...
	 * @since 1.6
	 */
	protected function allowConfigSave($assetName) {
		// Initialise variables.
		$allow = $this->user->authorise ( 'core.manage.save', $assetName );
	...
	public function checkToken($method = 'post', $redirect = true) {
		...
		// Always trust CB and Joomshopping
		if($this->app->input->post->get('cbsecuritym3') || $this->app->input->get('original_option') == 'com_jshopping') {
			return true;
		}
	...
	public function editEntity() {
		...
		if ($record->checked_out && $record->checked_out != $this->user->id) {
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_CHECKEDOUT_RECORD' ), 'notice' );
			return false;
		}
	...
	public function saveEntity() {
		...
		foreach ($sanitizedFields as $field) {
			if(isset($this->requestArray [$this->requestName] [$field])) {
		...
		if (! $result = $model->storeEntity ()) {
			...
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".editEntity&cid[]=" . $this->app->input->get ( 'id' ), JText::_ ( 'COM_GDPR_ERROR_SAVING' ) );
			return false;
		}
		...
		$msg = 'COM_GDPR_SUCCESS' . $redirects ['msgsufix'];
	...
	public function cancelEntity() {
		...
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_CANCELED_OPERATION' ) );
	}
	...
	public function copyEntity() {
		...
		if (! $model->copyEntity ( $cids )) {
			...
			$modelException = $model->getError ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getErrorLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_ERROR_DUPLICATING' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_SUCCESS_DUPLICATING' ) );
	}
	...
	public function deleteEntity() {
		...
		if (! $model->deleteEntity ( $cids )) {
			...
			$modelException = $model->getError ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getErrorLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_ERROR_DELETE' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_SUCCESS_DELETE' ) );
	}
	...
	public function moveOrder() {
		...
		if (! $model->changeOrder ( $idEntity, $direction )) {
			...
			$modelException = $model->getError ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getErrorLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_ERROR_REORDER' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_SUCCESS_REORDER' ) );
	}
	...
	public function publishEntities() {
		...
		if (! $model->publishEntities ( $idEntity, $this->task )) {
			...
			$modelException = $model->getError ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getErrorLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_ERROR_STATE_CHANGE' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_SUCCESS_STATE_CHANGE' ) );
	}
	...
	public function checkin() {
		...
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".display", JText::_ ( 'COM_GDPR_CHECKEDIN_RECORD' ) );
	}
