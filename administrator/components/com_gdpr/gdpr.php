...
// Access check.
if (!JFactory::getUser()->authorise('core.manage', 'com_gdpr')) {
	return JFactory::getApplication()->enqueueMessage(JText::_('COM_GDPR_ERROR_ALERT_NOACCESS_THIS_COMPONENT'), 'error');
}
...
GdprLoader::setup();
GdprLoader::registerPrefix('Gdpr', JPATH_COMPONENT . '/framework');
...
$jLang->load('com_gdpr', JPATH_COMPONENT_ADMINISTRATOR, 'en-GB', true, true);
if($jLang->getTag() != 'en-GB') {
	$jLang->load('com_gdpr', JPATH_ADMINISTRATOR, null, true, false);
	$jLang->load('com_gdpr', JPATH_COMPONENT_ADMINISTRATOR, null, true, false);
}
