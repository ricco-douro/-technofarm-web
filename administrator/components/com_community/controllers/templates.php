	    ...
	    if (!$jinput->post->get('uploadTheme', NULL, 'NONE')) {
	    	...
	    	if ($themefile['type'] == 'application/zip' || $themefile['type'] == 'application/x-zip-compressed' || $themefile['type'] == 'application/octet-stream') {
				...
				if (move_uploaded_file($themefile['tmp_name'], $tmp_dest)) {
					...
					if (file_exists($path)) {
						$mainframe->redirect(JRoute::_('index.php?option=com_community&view=templates', false) , JText::_('COM_COMMUNITY_THEME_UPLOADED'), 'message');
					} else {
						JFolder::delete($templatefolder);

						$mainframe->redirect(JRoute::_('index.php?option=com_community&view=templates', false) , JText::_('COM_COMMUNITY_THEME_INVALID_FILE'), 'error');
					}
				} else {
					$mainframe->redirect(JRoute::_('index.php?option=com_community&view=templates', false) , JText::_('COM_COMMUNITY_THEME_UNABLE_UPLOAD'), 'error');
				}
	    	} else {
	    		$mainframe->redirect(JRoute::_('index.php?option=com_community&view=templates', false) , JText::_('COM_COMMUNITY_THEME_INVALID_FILE'), 'error');
	    	}
