...
class JChatDic {
	...
	public function addRule($name, JChatDicRule $rule) {
	...
	public function getRule($name) {
		...
		return isset($this->rules['*']) ? $this->rules['*'] : new JChatDicRule;
	}

	public function create($component, array $args = array(), $callback = null, $forceNewInstance = false) {
		if ($component instanceof JChatDicInstance)
	...
	private function getParams(array $params = array(), array $newInstances = array()) {
		for ($i = 0; $i < count($params); $i++) {
			if ($params[$i] instanceof JChatDicInstance)
	...
	private function getMethodParams($className, $method, JChatDicRule $rule, array $args = array(), array $share = array()) {
		...
		foreach ($params as $param) {
			...
			if ($paramClassName && isset($rule->substitutions[$paramClassName]))
				$parameters[] = is_string($rule->substitutions[$paramClassName]) ? new JChatDicInstance($rule->substitutions[$paramClassName]) : $rule->substitutions[$paramClassName];
