...
class JChatModelMessages extends JChatModel implements IMessagesModel {
	...
	public function getFilters() {
		...
		$lists['type'] 	= JHTML::_('select.genericlist', $types, 'msg_type', 'class="inputbox hidden-phone" size="1" onchange="document.adminForm.task.value=\'messages.display\';document.adminForm.submit( );"', 'value', 'text', $this->state->get('msgType'));
		...
		$lists['status'] 	= JHTML::_('select.genericlist', $status, 'msg_status', 'class="inputbox hidden-phone" size="1" onchange="document.adminForm.task.value=\'messages.display\';document.adminForm.submit( );"', 'value', 'text', $this->state->get('msgStatus'));
